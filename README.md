...
is_signature_verified { 
  ...
  # Find index of aa_info for data_provider specified in payload of signedConsent
  input.SahamatiAAInfo[k].entityinfo.id == payload.DataProvider.id
  # Extract corresponding certs
  certificate := input.SahamatiAAInfo[k].entityinfo.certificate
  # Create jwks from certs
  jwks = json.marshal({ "keys": [certificate]})
  # Verify signature
  is_valid_signature(input.signedConsent, jwks)
}
