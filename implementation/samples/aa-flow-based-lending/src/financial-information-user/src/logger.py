...
def log_http_response(verb: str, latency_ms: float, size: float=0):
  if size == 0:
    log(f"{OKGREEN}{BOLD}Received {verb} response after {latency_ms:.1f} ms{END}")
  else:
    log(f"{OKGREEN}{BOLD}Received {verb} response after {latency_ms:.1f} ms ({size:.1f} KiB){END}")

def log_action(action: str, msg: str):
  log(f"{OKGREEN}{BOLD}{UNDERLINE}{action}{END}: {msg}")

def log_success(msg: str):
  log(f"{OKGREEN}{BOLD}{msg}{END}")

def log_info(msg: str):
  log(f"{OKBLUE}{BOLD}{msg}{END}")
